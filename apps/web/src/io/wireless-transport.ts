...
export class WirelessTransport {
  ...
  pump(): void {
    ...
    // Host: re-announce the beacon periodically (only while hosting, devid != 0).
    if (this.beaconTick++ % BEACON_EVERY === 0) {
    ...
    // Client: the game asked to connect to a discovered host → relay it.
  ...
  private onWire(raw: string): void {
    ...
    switch (msg.type) {
      ...
      case 'wl-connect': {
        // We're the host: register the client and tell it the assigned devid.
