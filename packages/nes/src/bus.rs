//! The CPU memory interface. The 6502 is byte-addressed, so the bus is just
//! `read8`/`write8` over a 16-bit address space. `Nes` (see `nes.rs`) is the
//! production implementor; CPU unit tests use a flat-RAM stub.
//!
