//! Crash screen — rendered straight into the SMS/GG RGBA8888 framebuffer when
//! the core detects a CPU deadlock (a HALT with interrupts disabled, which can
//! never be woken by the maskable INT), so the host shows a legible error
//! readout instead of a silent frozen frame. Self-contained: a tiny 5x7 bitmap
//! font + a dark-blue panel, drawn into the framebuffer bytes.
//!
//! Ported from the PS1 core's `crash.rs`. The PS1 framebuffer is `[u32]`; the
//! SMS framebuffer is a flat RGBA8888 *byte* slice, so this writes 4 bytes per
//! pixel (R,G,B,A, little-endian) instead of one packed word.

const BG: [u8; 4] = [0x10, 0x10, 0x60, 0xFF]; // dark blue
const FG: [u8; 4] = [0xFF, 0xFF, 0xFF, 0xFF]; // white

/// Render `lines` of text onto an RGBA8888 framebuffer of `w`×`h` pixels.
/// Fills the background dark blue, then blits white 5x7 glyphs at scale 1.
