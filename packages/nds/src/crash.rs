//! Crash screen — rendered into the top-screen framebuffer when the core
//! detects a fault loop (an unhandled-exception storm on the ARM9), so the host
//! shows a legible error readout instead of a silent black hang. Self-contained:
//! a tiny 5x7 bitmap font + a fixed dark-blue panel, drawn straight into the
//! presented RGBA8888 framebuffer.
//!
//! Ported from the PS1 core's `crash.rs`. The key difference: the PS1
//! framebuffer is a `[u32]` (one word per pixel); the NDS framebuffer is
//! RGBA8888 *bytes* (`[u8]`, four bytes per pixel), so glyphs are written as
//! byte quads here.

/// Dark-blue background, white foreground (RGBA8888 byte order: R, G, B, A).
const BG: [u8; 4] = [0x10, 0x10, 0x60, 0xFF];
const FG: [u8; 4] = [0xFF, 0xFF, 0xFF, 0xFF];

/// Render `lines` of text onto an RGBA8888 framebuffer of size `w`x`h`: fill the
/// whole buffer dark blue, then blit each line of white 5x7 glyphs (scale 1).
...
/// Blit one `SCALE`-magnified glyph at (x0,y0) into the RGBA8888 framebuffer.
