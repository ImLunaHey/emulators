...
//! Ported 1:1 from ../../ds-recomp/src/cpu/state.ts.
//!
//! Both cores have the same banked register layout — only the instruction
//! set + cache/CP15 differ — so a single `CpuState` type backs BOTH CPUs in
//! the `Nds` god-struct. The bank logic and condition-code helpers are
//! identical to the GBA core's `state.rs` (the DS reused them verbatim).
