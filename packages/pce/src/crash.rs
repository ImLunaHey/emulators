//! Crash screen — rendered straight into the PC Engine RGBA8888 framebuffer
//! when the core detects a CPU deadlock (a tight self-jump with interrupts
//! disabled that nothing can break out of), so the host shows a legible error
//! readout instead of a silent frozen frame. Self-contained: a tiny 5x7 bitmap
//! font + a dark-blue panel, drawn into the framebuffer bytes (4 bytes/pixel,
//! R,G,B,A little-endian). Mirrors the sibling cores' `crash.rs`.
