...
//! font + a fixed 320x240 blue panel, drawn straight into the RGBA framebuffer.

use crate::gpu::{Gpu, VRAM_HALFWORDS};

/// Crash-screen display size (a safe, always-valid PSX-ish mode).
pub const W: usize = 320;
pub const H: usize = 240;

const BG: u32 = rgb(0x10, 0x10, 0x60); // dark blue
const FG: u32 = rgb(0xFF, 0xFF, 0xFF); // white
...
pub fn render(gpu: &mut Gpu, lines: &[String]) {
    ...
    let fb: &mut [u32; VRAM_HALFWORDS] = &mut gpu.framebuffer;
    for px in fb[..W * H].iter_mut() {
        *px = BG;
    }
    ...
    for (li, line) in lines.iter().enumerate() {
        ...
        for (ci, ch) in line.bytes().enumerate() {
            ...
            draw_glyph(fb, x0, y0, ch, SCALE);
        }
    }
}

/// Blit one `SCALE`-magnified glyph at (x0,y0) into the framebuffer.
fn draw_glyph(fb: &mut [u32; VRAM_HALFWORDS], x0: usize, y0: usize, ch: u8, scale: usize) {
    let rows = glyph(ch);
