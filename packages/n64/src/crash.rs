//! Crash screen — rendered into the framebuffer when the core detects a CPU
...
//! font drawn straight into an RGBA8888 framebuffer (4 bytes/pixel,
...
mod tests {
    ...
    #[test]
    fn render_fills_background_and_does_not_panic() {
        let (w, h) = (320, 240);
        let mut fb = vec![0u8; w * h * 4];
        render(&mut fb, w, h, &["N64 CPU FAULT".to_string()]);
        assert_eq!(&fb[0..4], &BG);
    }
}
