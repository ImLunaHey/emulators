//! Crash screen — rendered into the TIA framebuffer when the core detects a CPU
//! fault (a 6502 JAM/KIL illegal opcode that hard-halts the processor), so the
//! host shows a legible error readout instead of a silent frozen frame.
//! Self-contained: a tiny 5x7 bitmap font drawn straight into the RGBA8888
//! framebuffer (4 bytes/pixel, R,G,B,A little-endian).
