//! MOS 6507 CPU core — a 6502 with a reduced (13-bit) address bus.
//!
//! Spec: 6502.org instruction reference + the Stella Programmer's Guide. The
//! 6507 used in the 2600 is electrically a 6502 with the top three address pins
//! and the interrupt pins removed: the instruction set, flags, and cycle counts
//! are identical to the NMOS 6502, so this core implements every official
//! opcode (correct flags + cycle counts, including page-cross and branch-taken
//! penalties) plus the common unofficial opcodes (LAX, SAX, DCP, ISC, SLO, RLA,
//! SRE, RRA, the NOP variants). The address truncation to 13 bits is done by
//! the bus implementor ([`crate::Atari`]), not here.
//!
//! The CPU drives memory through `&mut dyn Bus` (see [`crate::bus::Bus`]), so it
//! never knows which device backs an address. `step()` executes one instruction
//! and returns the cycle count it consumed; the orchestrator advances the TIA
//! by 3 colour clocks per CPU cycle.
//!
//! NMI/IRQ exist for completeness but the 2600 leaves both pins tied high — the
//! TIA/RIOT cannot assert them — so `nmi_pending`/`irq_line` stay false in
//! normal operation. RESET (vector at $FFFC) is used at power-on.
...
pub const FLAG_D: u8 = 1 << 3; // decimal (BCD; the 2600's 6507 *does* support it)
...
pub struct Cpu {
    ...
    /// Pending interrupt lines. The 2600 ties both pins high, so these are
    /// normally inert, but they're implemented for correctness/testing.
...
impl Cpu {
    ...
    /// Power-on / RESET: load PC from the reset vector, set I, sp = $FD.
    ...
    fn execute(&mut self, bus: &mut dyn Bus, op: u8) -> u64 {
        ...
        match op {
            ...
            // JAM / KIL: undocumented opcodes that hard-halt a real 6502. Latch
            // the fault and rewind PC to the opcode so the CPU stays wedged; the
            // orchestrator turns this into a core fault + crash screen.
    ...
    fn sbc(&mut self, v: u8) {
        ...
        let c = self.flag(FLAG_C) as u16;
        let inv = (v ^ 0xFF) as u16;
        let sum = self.a as u16 + inv + c;
        let result = sum as u8;
        self.set_flag(FLAG_C, sum > 0xFF);
        self.set_flag(FLAG_V, (self.a ^ result) & (inv as u8 ^ result) & 0x80 != 0);
        self.a = result;
        self.set_zn(result);
    }

    // NMOS 6502 decimal-mode add. N/Z reflect the *binary* result; V is the
    // binary overflow; C is the decimal carry.
    fn adc_bcd(&mut self, v: u8) {
        let c = self.flag(FLAG_C) as u16;
        let a = self.a as u16;
        let bin = a + v as u16 + c;
        // Z is set from the binary sum (NMOS quirk).
        self.set_flag(FLAG_Z, (bin as u8) == 0);

        let mut lo = (a & 0x0F) + (v as u16 & 0x0F) + c;
        if lo > 0x09 {
            lo += 0x06;
        }
        let mut hi = (a >> 4) + (v as u16 >> 4) + if lo > 0x0F { 1 } else { 0 };
        // N and V are evaluated before the high-nibble decimal adjust.
        let pre = ((hi << 4) | (lo & 0x0F)) as u8;
        self.set_flag(FLAG_N, pre & 0x80 != 0);
        self.set_flag(FLAG_V, (a as u8 ^ pre) & (v ^ pre) & 0x80 != 0);
        if hi > 0x09 {
            hi += 0x06;
        }
    ...
    // NMOS 6502 decimal-mode subtract.
    fn sbc_bcd(&mut self, v: u8) {
        let c = self.flag(FLAG_C) as i16;
        let a = self.a as i16;
        let bin = a - v as i16 - (1 - c);
        // N/Z/V come from the binary result.
        let bres = (a + (v as i16 ^ 0xFF) + c) as u16;
        self.set_flag(FLAG_Z, (bres as u8) == 0);
        self.set_flag(FLAG_N, bres as u8 & 0x80 != 0);
        self.set_flag(
            FLAG_V,
            (self.a ^ (bres as u8)) & ((v ^ 0xFF) ^ (bres as u8)) & 0x80 != 0,
        );

        let mut lo = (a & 0x0F) - (v as i16 & 0x0F) - (1 - c);
        if lo < 0 {
            lo -= 0x06;
        }
        let mut hi = (a >> 4) - (v as i16 >> 4) - if lo < 0 { 1 } else { 0 };
        if hi < 0 {
            hi -= 0x06;
        }
...
mod tests {
    ...
    #[test]
    fn decimal_adc() {
        // 0x19 + 0x28 = 0x47 in BCD, no carry.
        let mut bus = FlatBus::new();
        let mut cpu = Cpu::new();
        cpu.pc = 0xF000;
        cpu.a = 0x19;
    ...
    fn decimal_adc_carry() {
        // 0x58 + 0x46 = 0x04 with carry in BCD.
        let mut bus = FlatBus::new();
        let mut cpu = Cpu::new();
        cpu.pc = 0xF000;
        cpu.a = 0x58;
    ...
    #[test]
    fn zero_page_x_cycles() {
        let mut bus = FlatBus::new();
        let mut cpu = Cpu::new();
        cpu.pc = 0xF000;
        cpu.x = 0x05;
        bus.ram[0xF000] = 0xB5; // LDA $10,X
        bus.ram[0xF001] = 0x10;
        bus.ram[0x0015] = 0x77;
        let c = run_one(&mut cpu, &mut bus);
    ...
    #[test]
    fn abs_x_page_cross_penalty() {
        let mut bus = FlatBus::new();
        let mut cpu = Cpu::new();
        cpu.pc = 0xF000;
        cpu.x = 0x01;
        bus.ram[0xF000] = 0xBD; // LDA $80FF,X -> $8100 (page cross)
        bus.ram[0xF001] = 0xFF;
        bus.ram[0xF002] = 0x80;
        bus.ram[0x8100] = 0xAB;
        let c = run_one(&mut cpu, &mut bus);
        assert_eq!(cpu.a, 0xAB);
        assert_eq!(c, 5); // 4 + 1 page cross
    }
    ...
    fn branch_taken_page_cross_cycles() {
        ...
        bus.ram[0xF0F0] = 0xF0; // BEQ +0x20 -> crosses into 0xF112
        bus.ram[0xF0F1] = 0x20;
        let c = run_one(&mut cpu, &mut bus);
        assert_eq!(cpu.pc, 0xF112);
        assert_eq!(c, 4);
    }
    ...
    fn jsr_rts_roundtrip() {
        ...
        let mut cpu = Cpu::new();
        cpu.pc = 0xF000;
        bus.ram[0xF000] = 0x20; // JSR $F900
        bus.ram[0xF001] = 0x00;
        bus.ram[0xF002] = 0xF9;
        bus.ram[0xF900] = 0x60; // RTS
        run_one(&mut cpu, &mut bus);
        assert_eq!(cpu.pc, 0xF900);
        run_one(&mut cpu, &mut bus);
    ...
    fn indirect_jmp_page_bug() {
        ...
        let mut cpu = Cpu::new();
        cpu.pc = 0xF000;
        bus.ram[0xF000] = 0x6C; // JMP ($30FF)
        bus.ram[0xF001] = 0xFF;
        bus.ram[0xF002] = 0x30;
        bus.ram[0x30FF] = 0x40;
        bus.ram[0x3000] = 0x80; // bug
        bus.ram[0x3100] = 0x99;
        run_one(&mut cpu, &mut bus);
    ...
    #[test]
    fn jam_opcode_latches_fault() {
        let mut bus = FlatBus::new();
        let mut cpu = Cpu::new();
        cpu.pc = 0xF000;
        bus.ram[0xF000] = 0x02; // JAM
        run_one(&mut cpu, &mut bus);
        let (op, pc) = cpu.jam.expect("JAM should latch");
        assert_eq!(op, 0x02);
        assert_eq!(pc, 0xF000);
        assert_eq!(cpu.pc, 0xF000); // PC rewound to wedge
    }
