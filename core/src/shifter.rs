...
pub fn imm_shift(op: u32, imm: u32, value: u32, carry_in: u32) -> ShiftResult {
    match op {
        SHIFT_LSL => {
            ...
            ShiftResult { value: value << imm, carry: (value >> (32 - imm)) & 1 }
        }
        SHIFT_LSR => {
            if imm == 0 {
                return ShiftResult { value: 0, carry: (value >> 31) & 1 }; // LSR #32
            }
            ShiftResult { value: value >> imm, carry: (value >> (imm - 1)) & 1 }
        }
        SHIFT_ASR => {
            if imm == 0 {
                let s = (value as i32) >> 31;
                return ShiftResult { value: s as u32, carry: (value >> 31) & 1 };
            }
            ShiftResult {
        ...
        SHIFT_ROR => {
            if imm == 0 {
                // RRX
                let carry = value & 1;
                return ShiftResult { value: (carry_in << 31) | (value >> 1), carry };
            }
            ShiftResult {
        ...
        _ => ShiftResult { value, carry: carry_in },
    }
}
...
pub fn reg_shift(op: u32, amount: u32, value: u32, carry_in: u32) -> ShiftResult {
    ...
    match op {
        SHIFT_LSL => {
            if amount < 32 {
                return ShiftResult { value: value << amount, carry: (value >> (32 - amount)) & 1 };
            }
            if amount == 32 {
                return ShiftResult { value: 0, carry: value & 1 };
            }
            ShiftResult { value: 0, carry: 0 }
        }
        SHIFT_LSR => {
            if amount < 32 {
                return ShiftResult { value: value >> amount, carry: (value >> (amount - 1)) & 1 };
            }
            if amount == 32 {
                return ShiftResult { value: 0, carry: (value >> 31) & 1 };
            }
            ShiftResult { value: 0, carry: 0 }
        }
        SHIFT_ASR => {
            ...
            {
                let sign = (value >> 31) & 1;
                ShiftResult { value: if sign != 0 { 0xFFFFFFFF } else { 0 }, carry: sign }
            }
        }
        SHIFT_ROR => {
            let a = amount & 31;
            if a == 0 {
                return ShiftResult { value, carry: (value >> 31) & 1 };
            }
            ShiftResult {
