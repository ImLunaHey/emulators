...
export class Emulator {
    ...
    // CLI tests).
    installRtcInterposer() {
        const bus = this.bus;
        const rtc = this.rtc;
        const inRange = (a) => (a & 0xFFFFFFFE) === 0x080000C4 ||
        ...
        bus.write16 = (a, v) => { if (inRange(a))
            rtc.write(a & 0xFF, v);
        else
            oW16(a, v); };
        bus.read8 = (a) => inRange(a) ? rtc.read(a & 0xFF) : oR8(a);
        bus.write8 = (a, v) => { if (inRange(a))
            rtc.write(a & 0xFF, v);
        else
            oW8(a, v); };
    }
    ...
    runFrame() {
        ...
        while (executed < CYCLES_PER_FRAME) {
            ...
            if (batch > CYCLES_PER_FRAME - executed)
                batch = CYCLES_PER_FRAME - executed;
            if (batch <= 0)
                batch = 1;
            let i = 0;
            while (i < batch) {
