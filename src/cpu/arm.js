...
function adcSetFlags(s, a, b, cIn) {
    const sum = a + b + cIn;
    const r = sum >>> 0;
    s.setNZ(r);
...
function sbcSetFlags(s, a, b, cIn) {
...
export function armExecute(cpu, instr) {
    const s = cpu.state;
    const cond = (instr >>> 28) & 0xF;
    if (cond !== 0xE && !s.checkCond(cond))
        return;
    ...
    if ((instr & 0x0E000000) === 0x0A000000) {
        ...
        if (offset & 0x02000000)
            offset |= 0xFC000000;
        if (instr & 0x01000000)
            s.r[14] = (s.r[15] - 4) >>> 0; // BL: LR = pc+4 of next
...
// ---------------------------------------------------------------- data processing
function armDataProcessing(cpu, instr) {
    const s = cpu.state;
    ...
    switch (opcode) {
        ...
        case 0x1:
            ...
            break; // EOR
        case 0x2:
            result = setFlags ? subSetFlags(s, op1, op2) : (op1 - op2) >>> 0;
            break; // SUB
        case 0x3:
            result = setFlags ? subSetFlags(s, op2, op1) : (op2 - op1) >>> 0;
            break; // RSB
        case 0x4:
            result = setFlags ? addSetFlags(s, op1, op2) : (op1 + op2) >>> 0;
            break; // ADD
        case 0x5:
            result = setFlags ? adcSetFlags(s, op1, op2, cIn) : (op1 + op2 + cIn) >>> 0;
            break; // ADC
        case 0x6:
            result = setFlags ? sbcSetFlags(s, op1, op2, cIn) : (op1 - op2 - (cIn ^ 1)) >>> 0;
            break; // SBC
        case 0x7:
            result = setFlags ? sbcSetFlags(s, op2, op1, cIn) : (op2 - op1 - (cIn ^ 1)) >>> 0;
            break; // RSC
        case 0x8:
            writeResult = false;
            result = (op1 & op2) >>> 0;
        ...
        case 0xA:
            writeResult = false;
            subSetFlags(s, op1, op2);
            break; // CMP
        case 0xB:
            writeResult = false;
            addSetFlags(s, op1, op2);
            break; // CMN
        case 0xC:
            result = (op1 | op2) >>> 0;
            if (setFlags) {
...
function applyMsr(s, isSpsr, instr, val) {
    let mask = 0;
    if (instr & 0x00010000)
    ...
    if ((newMode !== s.mode()))
        s.switchMode(newMode);
    s.cpsr = newCpsr >>> 0;
}
// ---------------------------------------------------------------- single transfer LDR/STR
function armSingleTransfer(cpu, instr) {
    const s = cpu.state;
    ...
    if (L) {
        ...
        let val = s.r[rd];
        if (rd === 15)
            val = (val + 4) >>> 0; // STR Rd=PC stores pc+12 of original instr
        if (B)
            cpu.bus.write8(addr, val & 0xFF);
        else
            cpu.bus.write32(addr & ~3, val >>> 0);
        if (writeback)
            s.r[rn] = eff >>> 0;
    }
}
// ---------------------------------------------------------------- halfword / signed transfer
function armHalfTransfer(cpu, instr) {
    const s = cpu.state;
    const P = (instr & 0x01000000) !== 0;
    const U = (instr & 0x00800000) !== 0;
...
// ---------------------------------------------------------------- multiply
function armMultiply(cpu, instr) {
    const s = cpu.state;
...
function armBlockTransfer(cpu, instr) {
    const s = cpu.state;
    const P = (instr & 0x01000000) !== 0;
    const U = (instr & 0x00800000) !== 0;
    ...
    if (count === 0) {
        ...
        if (L) {
            s.r[15] = cpu.bus.read32(s.r[rn] & ~3);
            cpu.flushPipeline();
        }
        else {
            cpu.bus.write32(s.r[rn] & ~3, s.r[15]);
        }
        if (W)
            s.r[rn] = U ? (s.r[rn] + 0x40) >>> 0 : (s.r[rn] - 0x40) >>> 0;
        return;
    }
    let base = s.r[rn];
    ...
    if (userBank)
        s.switchMode(savedMode);
    if (W)
        s.r[rn] = writebackAddr;
}
