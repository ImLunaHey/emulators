...
export function thumbExecute(cpu, instr) {
    const s = cpu.state;
    const top = instr >>> 13;
    switch (top) {
        case 0b000: {
        ...
        case 0b001: {
            ...
            switch (op) {
                case 0:
                    ...
                    return;
                case 1:
                    subSetFlags(s, s.r[rd], imm);
                    return;
                case 2:
                    s.r[rd] = addSetFlags(s, s.r[rd], imm);
                    return;
                case 3:
                    s.r[rd] = subSetFlags(s, s.r[rd], imm);
                    return;
            }
            return;
        }
        case 0b010: {
            ...
            if (high4 === 0b0101) {
                ...
                if (bit9 === 0) {
                    ...
                    if (L) {
                        ...
                        if (B)
                            cpu.bus.write8(addr, s.r[rd] & 0xFF);
                        else
            ...
            if (((instr >>> 10) & 7) === 0b000) {
                ...
                switch (op) {
                    ...
                    case 0x2: {
                        const r = regShift(0, b & 0xFF, a, cIn);
                        s.r[rd] = r.value >>> 0;
                        s.setNZ(r.value);
                        applyCarry(s, r.carry);
                        return;
                    ...
                    case 0x3: {
                        const r = regShift(1, b & 0xFF, a, cIn);
                        s.r[rd] = r.value >>> 0;
                        s.setNZ(r.value);
                        applyCarry(s, r.carry);
                        return;
                    ...
                    case 0x4: {
                        const r = regShift(2, b & 0xFF, a, cIn);
                        s.r[rd] = r.value >>> 0;
                        s.setNZ(r.value);
                        applyCarry(s, r.carry);
                        return;
                    ...
                    case 0x5:
                        s.r[rd] = adcSetFlags(s, a, b, cIn);
                        return;
                    case 0x6:
                        s.r[rd] = sbcSetFlags(s, a, b, cIn);
                        return;
                    case 0x7: {
                        const r = regShift(3, b & 0xFF, a, cIn);
                        s.r[rd] = r.value >>> 0;
                        s.setNZ(r.value);
                        applyCarry(s, r.carry);
                        return;
        ...
        case 0b101: {
            ...
            if ((instr & 0x0600) === 0x0400) {
                ...
                if (L) {
                    ...
                    for (let i = 0; i < 8; i++) {
                        if (list & (1 << i)) {
                            cpu.bus.write32(sp & ~3, s.r[i] >>> 0);
                            sp = (sp + 4) >>> 0;
                        }
                    }
                    if (R) {
                        cpu.bus.write32(sp & ~3, s.r[14] >>> 0);
                    }
                    s.r[13] = start;
                }
                return;
        ...
        case 0b110: {
            if ((instr & 0x1000) === 0) {
                ...
                if (list === 0) {
                    ...
                    if (L) {
                        s.r[15] = cpu.bus.read32(addr & ~3);
                        cpu.flushPipeline();
                    }
                    else {
                        cpu.bus.write32(addr & ~3, s.r[15]);
                    }
                    s.r[rb] = (addr + 0x40) >>> 0;
                    return;
                }
        ...
        case 0b111: {
            ...
            if (H === 0b11) {
                ...
                const tg = newPc & ~1;
                if (globalThis.__traceBL)
                    globalThis.__traceBL(s.r[15] - 4, tg);
                s.r[15] = tg;
                s.r[14] = newLr;
                cpu.flushPipeline();
                return;
