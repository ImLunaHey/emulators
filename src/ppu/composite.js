...
//   bit 21       OBJ window
function bgr555ToRgba(bgr, out, off) {
    const r = bgr & 0x1F;
    const g = (bgr >> 5) & 0x1F;
    const b = (bgr >> 10) & 0x1F;
    out[off] = (r << 3) | (r >> 2);
...
function bgr555Blend(a, b, eva, evb) {
...
export function compositeScanline(ppu, y, backdrop) {
    const out = ppu.frame;
    ...
    for (let x = 0; x < 240; x++) {
        ...
        if (anyWinEn) {
            ...
            if (inW0)
                allowMask = w0InEnable;
            else if (inW1)
                allowMask = w1InEnable;
            else if (inObjWin)
                allowMask = wObjInEnable;
            else
                allowMask = wOutEnable;
        }
