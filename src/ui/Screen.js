...
export function Screen({ emu, paused, audio, onStats }) {
    ...
    useEffect(() => {
        ...
        const loop = (ts) => {
            ...
            while (accumMs >= GBA_FRAME_MS) {
                ...
                if (samples.length > 0)
                    audio.push(samples, emu.sound.sampleRate);
            }
            if (didFrame) {
            ...
            if (sinceStat >= 500) {
                ...
                onStats(`${fpsAvg.toFixed(1)} fps · ${(280896 * fpsAvg / 1e6).toFixed(2)} MHz · jit ${jitPct}%`);
            }
        };
        raf = requestAnimationFrame(loop);
        return () => { stop = true; cancelAnimationFrame(raf); };
    }, [emu, paused, onStats]);
    return _jsx("canvas", { ref: canvasRef, id: "screen", width: 240, height: 160 });
}
