...
export class BiosHle {
    ...
    handleSwi(comment) {
        ...
        switch (comment) {
            ...
            case 0x10:
                ...
                return true;
            case 0x11:
                this.lz77UnComp(false);
                return true;
            case 0x12:
                this.lz77UnComp(true);
                return true;
            case 0x13:
                this.huffUnComp();
                return true;
    ...
    registerRamReset(mask) {
        ...
        if (mask & 0x02)
            this.bus.iwram.fill(0, 0, 0x7E00); // BIOS leaves stack area
        if (mask & 0x04)
            this.bus.pram.fill(0);
        if (mask & 0x08)
        ...
        if (mask & 0x80) {
            ...
            const ppu = this.cpu.bus.io?.ppu;
            if (ppu) {
                ppu.bgPA[0] = 0x100;
                ppu.bgPD[0] = 0x100; // BG2 identity
                ppu.bgPA[1] = 0x100;
                ppu.bgPD[1] = 0x100; // BG3 identity
                ppu.bgPB[0] = 0;
                ppu.bgPC[0] = 0;
                ppu.bgPB[1] = 0;
                ppu.bgPC[1] = 0;
    ...
    // -------- Interrupt waits --------
    intrWait(discardOld, wanted) {
        const irq = this.cpu.bus.io && this.cpu.bus.io.irq;
        if (!irq)
            return;
        if (discardOld)
            irq.iflag &= ~wanted;
    ...
    bgAffineSet() {
        ...
        for (let i = 0; i < n; i++) {
            ...
            this.bus.write16(dst, pa);
            this.bus.write16(dst + 2, pb);
            this.bus.write16(dst + 4, pc);
            this.bus.write16(dst + 6, pd);
    ...
    objAffineSet() {
        ...
        for (let i = 0; i < n; i++) {
            ...
            src += 8;
            const cos = Math.cos(ang), sin = Math.sin(ang);
            this.bus.write16(dst, Math.round(sx * cos) & 0xFFFF);
            dst += off;
            this.bus.write16(dst, Math.round(-sx * sin) & 0xFFFF);
            dst += off;
            this.bus.write16(dst, Math.round(sy * sin) & 0xFFFF);
            dst += off;
            this.bus.write16(dst, Math.round(sy * cos) & 0xFFFF);
            dst += off;
        }
    }
    // -------- BitUnPack --------
    ...
    lz77UnComp(vram) {
        ...
        const writeByte = (b) => {
            ...
            if (halfBufHas === 0) {
                ...
                this.bus.write16(dst, halfBuf | (b << 8));
                dst = (dst + 2) >>> 0;
                halfBufHas = 0;
            }
        };
        while (length > 0) {
            let flags = this.bus.read8(src);
            src = (src + 1) >>> 0;
            for (let i = 0; i < 8 && length > 0; i++) {
                if (flags & 0x80) {
                    const a = this.bus.read8(src);
                    src = (src + 1) >>> 0;
                    const b = this.bus.read8(src);
                    src = (src + 1) >>> 0;
                    const len = ((a >> 4) & 0xF) + 3;
    ...
    diff16() {
        ...
        src = (src + 4) >>> 0;
        let prev = this.bus.read16(src);
        src = (src + 2) >>> 0;
        this.bus.write16(dst, prev);
        dst = (dst + 2) >>> 0;
        length--;
        while (length > 0) {
            const d = this.bus.read16(src);
            src = (src + 2) >>> 0;
            prev = (prev + d) & 0xFFFF;
            this.bus.write16(dst, prev);
            dst = (dst + 2) >>> 0;
