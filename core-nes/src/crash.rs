...
//! CPU fault (a 6502 JAM/KIL illegal opcode that hard-halts the processor), so
//! the host shows a legible error readout instead of a silent frozen frame.
//! Self-contained: a tiny 5x7 bitmap font drawn straight into the 256x240
//! RGBA8888 framebuffer (4 bytes/pixel, R,G,B,A little-endian).

const BG: [u8; 4] = [0x10, 0x10, 0x60, 0xFF]; // dark blue
const FG: [u8; 4] = [0xFF, 0xFF, 0xFF, 0xFF]; // white

/// Render `lines` of text onto an RGBA8888 framebuffer of size `w`x`h`:
/// a dark-blue background with white 5x7 text at integer scale 1.
pub fn render(fb: &mut [u8], w: usize, h: usize, lines: &[String]) {
    // Dark-blue background.
    for px in fb[..w * h * 4].chunks_exact_mut(4) {
...
/// Blit one `scale`-magnified glyph at (x0,y0) into the RGBA framebuffer.
