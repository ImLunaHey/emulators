//! Crash screen — rendered straight into the WonderSwan RGBA8888 framebuffer
//! when the core detects a CPU fault (an undefined opcode the V30MZ can't
//! execute), so the host shows a legible error readout instead of a silent
//! frozen frame. Self-contained: a tiny 5x7 bitmap font + a dark-blue panel.
//!
//! Adapted from the sibling cores' `crash.rs`. The framebuffer is a flat
//! RGBA8888 byte slice, so this writes 4 bytes per pixel (R,G,B,A).
